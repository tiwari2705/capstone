...
router.get('/users', async (req, res) => {
  try {
    ...
    const params = [];
    let paramCount = 1;
    if (course) { params.push(course); whereConditions.push('course = $' + paramCount++); }
    if (section) { params.push(section); whereConditions.push('section = $' + paramCount++); }
    if (year_of_passing) { params.push(parseInt(year_of_passing)); whereConditions.push('year_of_passing = $' + paramCount++); }
    if (search) { params.push('%' + search + '%'); whereConditions.push('(registration_no ILIKE $' + paramCount + ' OR name ILIKE $' + paramCount + ' OR email ILIKE $' + paramCount + ')'); paramCount++; }
    ...
    const query = 'SELECT u.id, u.name, u.email, u.registration_no, u.course, u.section, u.year_of_passing, u.created_at, COUNT(DISTINCT cp.id) as verified_profiles, COALESCE(SUM(s.problems_solved), 0) as total_problems, COALESCE(SUM(s.score), 0) as total_score FROM users u LEFT JOIN coding_profiles cp ON u.id = cp.user_id AND cp.verified = TRUE LEFT JOIN stats s ON u.id = s.user_id ' + whereClause + ' GROUP BY u.id ORDER BY ' + sortColumn + ' ' + sortOrder + ' LIMIT $' + paramCount++ + ' OFFSET $' + paramCount++;
    const result = await pool.query(query, params);
    const countQuery = 'SELECT COUNT(*) as total FROM users u ' + whereClause;
    const countResult = await pool.query(countQuery, params.slice(0, -2));
    res.json({ users: result.rows, total: parseInt(countResult.rows[0].total), limit: parseInt(limit), offset: parseInt(offset) });
  } catch (err) {
...
router.get('/users/:identifier', async (req, res) => {
  try {
    ...
    res.json({ user, profiles: profilesResult.rows, stats: statsMap, totalProblems, score: totalScore.toFixed(2) });
  } catch (err) {
...
router.get('/section/:section', async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT u.id, u.name, u.email, u.registration_no, u.course, u.section, COUNT(DISTINCT cp.id) as verified_profiles, COALESCE(SUM(s.problems_solved), 0) as total_problems, COALESCE(SUM(s.score), 0) as total_score, COALESCE(AVG(s.rating), 0) as avg_rating FROM users u LEFT JOIN coding_profiles cp ON u.id = cp.user_id AND cp.verified = TRUE LEFT JOIN stats s ON u.id = s.user_id WHERE u.section = $1 AND u.role = 'user' GROUP BY u.id ORDER BY total_problems DESC`, [section]);
    const sectionStats = { totalStudents: result.rows.length, totalProblems: result.rows.reduce((sum, u) => sum + parseInt(u.total_problems), 0), avgProblems: result.rows.length > 0 ? (result.rows.reduce((sum, u) => sum + parseInt(u.total_problems), 0) / result.rows.length).toFixed(2) : 0, topPerformer: result.rows[0] || null };
...
router.get('/course/:course', async (req, res) => {
  try {
    ...
    const result = await pool.query(`SELECT u.id, u.name, u.email, u.registration_no, u.course, u.section, COUNT(DISTINCT cp.id) as verified_profiles, COALESCE(SUM(s.problems_solved), 0) as total_problems, COALESCE(SUM(s.score), 0) as total_score, COALESCE(AVG(s.rating), 0) as avg_rating FROM users u LEFT JOIN coding_profiles cp ON u.id = cp.user_id AND cp.verified = TRUE LEFT JOIN stats s ON u.id = s.user_id WHERE u.course = $1 AND u.role = 'user' GROUP BY u.id ORDER BY total_problems DESC`, [course]);
    const courseStats = { totalStudents: result.rows.length, totalProblems: result.rows.reduce((sum, u) => sum + parseInt(u.total_problems), 0), avgProblems: result.rows.length > 0 ? (result.rows.reduce((sum, u) => sum + parseInt(u.total_problems), 0) / result.rows.length).toFixed(2) : 0, topPerformer: result.rows[0] || null };
    const sectionBreakdown = await pool.query(`SELECT u.section, COUNT(DISTINCT u.id) as student_count, COALESCE(SUM(s.problems_solved), 0) as total_problems FROM users u LEFT JOIN stats s ON u.id = s.user_id WHERE u.course = $1 AND u.role = 'user' GROUP BY u.section ORDER BY u.section`, [course]);
    res.json({ course, stats: courseStats, sectionBreakdown: sectionBreakdown.rows, students: result.rows });
  } catch (err) {
...
router.get('/leaderboard', async (req, res) => {
  try {
    ...
    const params = [];
    let paramCount = 1;
    if (course) { params.push(course); whereConditions.push('u.course = $' + paramCount++); }
    if (section) { params.push(section); whereConditions.push('u.section = $' + paramCount++); }
    ...
    const query = 'SELECT u.id, u.name, u.email, u.registration_no, u.course, u.section, COALESCE(SUM(s.problems_solved), 0) as total_problems, COALESCE(SUM(s.score), 0) as total_score, COALESCE(AVG(s.rating), 0) as avg_rating, COUNT(DISTINCT cp.id) as verified_profiles FROM users u LEFT JOIN coding_profiles cp ON u.id = cp.user_id AND cp.verified = TRUE LEFT JOIN stats s ON u.id = s.user_id WHERE ' + whereClause + ' GROUP BY u.id ORDER BY ' + orderBy + ' LIMIT $' + paramCount;
    const result = await pool.query(query, params);
    const leaderboard = result.rows.map((row, index) => ({ ...row, rank: index + 1 }));
    res.json({ leaderboard, filters: { course, section, metric }, total: leaderboard.length });
  } catch (err) {
